...
function Reservation() {
    ...
    const [searchPriceRoomBread, setsearchPriceRommBread] = useState("");
    const [searchPriceBreak, setsearchPriceBreak] = useState("");
...
const handleChangeRating = event => {
    ...
    const filteredData = listOfHotel.filter(item =>
        // item.starrating.toLowerCase().includes(searchRating.toLowerCase()) ||
        item.starrating.toString().toLowerCase().includes(searchRating.toLowerCase()) ||
        item.starrating.toString().toUpperCase().includes(searchRating.toString().toUpperCase())
...
const handleChangePrice = event => {
    ...
    const filteredData = listOfHotel.filter(item =>
        item.priceperroom.toString().toLowerCase().includes(searchPrice.toLowerCase()) ||
        item.priceperroom.toString().toUpperCase().includes(searchPrice.toString().toUpperCase())
...
//for room bread 
const handleChangePriceRoomBread = event => {
    setsearchPriceRommBread(event.target.value);
    const filteredData = listOfHotel.filter(item =>
        item.priceperroombread.toString().toLowerCase().includes(searchPriceRoomBread.toLowerCase()) ||
        item.priceperroombread.toString().toUpperCase().includes(searchPriceRoomBread.toString().toUpperCase())
...
const handleChangePriceBreak = event => {
    setsearchPriceBreak(event.target.value);
    const filteredData = listOfHotel.filter(item =>
        item.priceperroombreakfast.toString().toLowerCase().includes(searchPriceBreak.toLowerCase()) ||
        item.priceperroombreakfast.toString().toUpperCase().includes(searchPriceBreak.toString().toUpperCase())
    ...
    useEffect(() => {
        ...
        if (destiniValue) {
            Axios.get("http://localhost:8080/hotels/reservation", {
                params:
                {
    ...
    const handleSubmit = (event) => {
        ...
        Axios.post("http://localhost:8080/hotels/checkout", { ...checkoutData })
            .then((response) => {
            ...
            <center>
                <table>
                    <tr>
                        ...
                        <td><input type="text" id="txtfilrating" value={searchRating} onChange={handleChangeRating} name="txtfilrating" /></td>
                         <td><label for="">Price</label></td>
                        <td><input type="text" id="txtprice" value={searchPrice} onChange={handleChangePrice} name="txtprice" placeholder="Full Board"/></td>
                        <td><input type="text" id="txtbreadprice" value={searchPriceRoomBread} onChange={handleChangePriceRoomBread} name="txtbreadprice" placeholder="Bread"/></td>
                        <td><input type="text" id="txtbreakfastprice" value={searchPriceBreak} onChange={handleChangePriceBreak} name="txtbreakfastprice" placeholder="Break Fast"/></td>
    </tr> 
                </table>
            </center>
            <br />
