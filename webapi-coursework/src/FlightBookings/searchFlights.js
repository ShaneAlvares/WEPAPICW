...
const SearchFlights = () => {
  ...
  const HandleSearchFunction = () => {
    ...
    if (cabinClass != "" && cabinClass != "Select") {
      searchBy.class = cabinClass;
    }
    if (airline != "" && airline != "Select") {
      searchBy.airline = airline;
    }
    // console.log(searchBy);

    axios
      .post("http://localhost:8080/flights/getFlights", searchBy)
      .then((response) => {
        ...
        if (
          ...
          setDepartureFlightItems([]);
          setArrivalFlightItems([]);
        } else {
          setErrorMsg("");
